 ...
 BEGIN_PROVIDER [ double precision, factor_een_blas_tiled ]
&BEGIN_PROVIDER [ double precision, factor_een_deriv_e_blas_tiled, (nelec_16,4) ]
 implicit none
 BEGIN_DOC
...
! double precision,dimension(:),allocatable :: cn
 ...
 factor_een_blas_tiled = 0.0d0
 factor_een_deriv_e_blas_tiled(1:nelec_16,1:4) = 0.0d0
   ...
   do a = 0, ntiles_nnuc - 1
    do aa = 1, tile_size
      ...
      do j = 0, ntiles_nelec - 1
        ...
        enddo
      enddo
      factor_een_blas_tiled = factor_een_blas_tiled + accu * cn

      do j = 0, ntiles_nelec - 1
        do ii=1,4
        do jj = 1, tile_size
          idxj = j*tile_size + jj
          factor_een_deriv_e_blas_tiled(idxj,ii) = factor_een_deriv_e_blas_tiled(idxj,ii) + (&
            tmp_c_tiled(jj,aa,m,j,a,k) * rescale_een_n_deriv_e(idxj,ii,idxa,m+l) +     &
            dtmp_c_tiled(jj,aa,ii,m,j,a,k)   * rescale_een_n_tiled(jj,aa,m+l,j,a) +            &
            dtmp_c_tiled(jj,aa,ii,m+l,j,a,k) * rescale_een_n_tiled(jj,aa,m  ,j,a) +            &
